...
export class LoginService {
  ...
  authenticateModel: AuthenticateModel | any;
  authenticateResult: AuthenticateResultModel | any;
  ...
  localization: LocalizationService | undefined;
  private MSAL: PublicClientApplication | undefined;
  ...
  authenticate(finallyCallback: () => void, redirectUrl?: string, captchaResponse?: string): void {
    ...
    this._localStorageService.getItem(
      ...
      function (err: any, value: { token: string | undefined }) {
        ...
        self._tokenAuthService.authenticate(self.authenticateModel).subscribe({
          ...
          error: (err: any) => {
            self._toastr.error('Failed login');
            // abp.event.trigger('showModal', {
            //   title: 'Server Error',
            //   content: `
            //   <p class="text-center">
            //     We couldn't process the request successfully.
            //   </p>
            //   `,
            //   buttonText: 'OK',
  ...
  public wsFederationLoginStatusChangeCallback(errorDesc: any, token: string, error: any, tokenType: any) {
  ...
  mobileGoogleLogin(access_token: string) {
    const model = new ExternalAuthenticateModel();
    model.authProvider = 'Google-Mobile';
    model.accessToken = access_token;
    model.providerKey = 'none';
    model.providerAccessCode = 'none';
    model.singleSignIn = UrlHelper.getSingleSignIn();
    model.returnUrl = UrlHelper.getReturnUrl();
  ...
  mobileAppleLogin(access_token: string) {
    const model = new ExternalAuthenticateModel();
    model.authProvider = 'Apple-Mobile';
    model.accessToken = access_token;
    model.providerKey = 'none';
    model.providerAccessCode = 'none';
    model.singleSignIn = UrlHelper.getSingleSignIn();
  ...
  private processAuthenticateResult(authenticateResult: AuthenticateResultModel, redirectUrl?: string) {
    ...
    if (authenticateResult.shouldResetPassword) {
      ...
      // Unexpected result!
      this._toastr.error('Failed login');
  ...
  private getWsFederationConnectConfig(loginProvider: ExternalLoginProvider): any {
    ...
    if (loginProvider.additionalParams && loginProvider.additionalParams['Tenant']) {
  ...
  private getOpenIdConnectConfig(loginProvider: ExternalLoginProvider): AuthConfig {
    ...
    if (loginProvider.additionalParams != undefined) {
  ...
  private facebookLoginStatusChangeCallback(resp: { status: any; authResponse?: any }) {
  ...
  private googleLoginStatusChangeCallback(response: any) {
    if (response.error !== undefined) {
      return;
    }
    var _$this = this;

    gapi.client.oauth2.userinfo.get().execute(function (resp: { id: string }) {
      const model = new ExternalAuthenticateModel();
