...
export class ChatSignalrService extends AppComponentBase {
  chatHub: HubConnection | undefined;
  ...
  registerChatEvents(connection: any): void {
    connection.on('getChatMessage', (message: any) => {
      abp.event.trigger('app.chat.messageReceived', message);
    });

    connection.on('getAllFriends', (friends: any) => {
      abp.event.trigger('abp.chat.friendListChanged', friends);
    });

    connection.on('getFriendshipRequest', (friendData: any, isOwnRequest: any) => {
      abp.event.trigger('app.chat.friendshipRequestReceived', friendData, isOwnRequest);
    });

    connection.on('getUserConnectNotification', (friend: any, isConnected: any) => {
    ...
    connection.on('getUserStateChange', (friend: any, state: any) => {
    ...
    connection.on('getallUnreadMessagesOfUserRead', (friend: any) => {
      abp.event.trigger('app.chat.allUnreadMessagesOfUserRead', {
        friend: friend,
      });
    });

    connection.on('getReadStateChange', (friend: any) => {
  ...
  sendMessage(messageData: any, callback: () => void): void {
    ...
    this.chatHub!.invoke('sendMessage', messageData)
      .then((result) => {
        if (result) {
  ...
  init(): void {
    this._zone.runOutsideAngular(() => {
      abp.signalr.connect();
      abp.signalr
        .startConnection(abp.appPath + 'signalr-chat', (connection: any) => {
